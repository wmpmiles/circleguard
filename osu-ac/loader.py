...
from exceptions import InvalidArgumentsException, APIException, AnticheatException, UnkownAPIException, InternalAPIException, RetryException, RatelimitException
...
def request(function):
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            print(*args)
            print("enforcing ratelimit but not really (debugging so I don't have to wait the 54 second sleep to get to a print statement and crash anyway")
            # Loader.enforce_ratelimit()
            # wrap with the decorator then call decorator function
            # (that's what the outside parens are for)
            print(type(request))
